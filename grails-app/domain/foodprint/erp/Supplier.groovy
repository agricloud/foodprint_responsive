...
class Supplier {
    ...
    public String toString() {
        ...
        "${getMessageSource().getMessage("${i18nType}.supplier.label", args, Locale.getDefault())}: " +
        "${name}-${title}"
    }

    static validator = { col, obj ->
        if (obj.tel && !(obj.tel ==~ /^((\+\d+\-)|(\(\d+\)))?(\d+[-]?)+( [#]\d+)*$/)) {
            return ["supplier.tel.invalid"]
        }

        if (obj.fax && !(obj.fax ==~ /^((\+\d+\-)|(\(\d+\)))?(\d+[-]?)+( [#]\d+)*$/)) {
            return ["supplier.fax.invalid"]
        }

        if (obj.email && !(obj.email ==~ /^[_a-zA-Z0-9-]+([.][_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+([.][a-zA-Z0-9-]+)*$/)) {
            return ["supplier.email.invalid"]
        }
    }
}
